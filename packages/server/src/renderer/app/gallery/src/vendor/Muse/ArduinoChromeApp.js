...
var Serial = function() {
  ...
  _this.send = (arr) => {
    ...
    if (_this.isOpen) {
      ...
      chrome.serial.send(_this.connectionId, new ArrayBuffer(arr), function() {}); //convertStringToArrayBuffer(str + '\n')
    }
  };
    ...
    var onReceiveCallback = function(info) {
      if (info.connectionId == _this.connectionId && info.data) {
        ...
        if (str.length > 1 && (str.charAt(str.length - 1) === '\n' || str.charAt(str.length - 1) === '\n')) {
          ...
          if (stringReceived == 'init') _this.onConnect();
          _this.onMessage(stringReceived);
          stringReceived = '';
